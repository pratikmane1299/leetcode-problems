function unqiue(arr) {
	...
	const countsArr = Array.from(map.values());

	for (let i = 0; i < countsArr.length; i++) {
		for (let j = i + 1; j < countsArr.length; j++) {
			if (countsArr[i] === countsArr[j]) {
				return false;
			} else continue;
		}
  }
...
console.log(unqiue([1, 2, 2, 1, 1, 3]));
console.log(unqiue([1, 2]));
console.log(unqiue([-3, 0, 1, -3, 1, 1, 1, -3, 10, 0]));
console.log(unqiue([3,5,-2,-3,-6,-6]));
